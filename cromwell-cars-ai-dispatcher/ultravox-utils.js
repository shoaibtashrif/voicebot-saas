...
export async function createUltravoxCall(callConfig) {
  ...
  return new Promise((resolve, reject) => {
    ...
    request.on('response', (response) => {
      ...
      response.on('end', () => {
        console.log(`📡 Ultravox Response Status: ${response.statusCode}`);
        console.log(`📦 Ultravox Response Body: ${data}`);
        ...
        try {
          ...
          reject(new Error(`Failed to parse Ultravox response: ${e.message}`));
        }
      });
    });

    request.on('error', (err) => {
      console.error('❌ Ultravox Request Error:', err);
      reject(err);
    });
